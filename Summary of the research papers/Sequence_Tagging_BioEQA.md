...
The proposed sequence tagging method avoids much of that complexity and makes the system more end-to-end. 
...
4. The final answer spans are reconstructed from the predicted tags. 

The authors mainly use a simple **linear layer** on top of the language model, although they also test **BiLSTM** and **BiLSTM-CRF** tagging layers. Interestingly, the simple linear layer works best overall, which suggests that a more complicated tagging head is not necessarily better for this QA setup. 

---

## Datasets and Experiments

The experiments were done on **BioASQ 7b** and **BioASQ 8b** list-type question datasets. The evaluation metric was mainly **F1 score** for list questions. The authors also used **sequential transfer learning**, first starting from models already trained on SQuAD and then fine-tuning them on BioASQ. 

A key result of the paper is that the sequence tagging approach consistently improved performance over earlier strong baselines for list-type biomedical QA. The paper reports average **F1 improvements of 3.80% on BioASQ 7b and 6.22% on BioASQ 8b** over the baseline model from Yoon et al. (2019b). 

Another thing I noticed is that the model especially improves **recall**. This makes sense because the tagging-based approach is better at finding **multiple correct answers** instead of being restricted to one answer or a threshold-limited number of answers. 
...
- The method also makes it easier to build a **unified model** that can handle both **factoid** and **list** questions. 
...
This paper shows that **sequence tagging is a better way to solve biomedical extractive question answering**, especially when questions need **multiple answer spans instead of just one**. 
