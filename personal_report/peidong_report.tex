...
\begin{enumerate}
  \item \textbf{TF-IDF + LR Baselines (A1a/b/c):} implemented three input variants (Q, Q+Ctx, Q+Ctx+Ans) to establish performance ceilings for sparse representations (\texttt{baseline\_lr\_group.py}).
...
(\texttt{team16-task1}), using feature branches and pull requests to integrate
contributions without conflicts.  A shared \textbf{Notion board} tracked task
ownership, deadlines, and blockers; weekly online check-ins via
...
results were cross-checked against the shared \texttt{dataset\_splits.json} file
to guarantee reproducibility.  A fixed random seed (\texttt{SEED = 42}) was
